...
static void __buf_init_done(void* user, struct qvio_user_job_done* user_job_done) {
	...
	buf->buf_init.dma_buf = user_job_done->u.buf_init.dma_buf;
	memcpy(buf->buf_init.offset, user_job_done->u.buf_init.offset, sizeof(user_job_done->u.buf_init.offset));
	memcpy(buf->buf_init.pitch, user_job_done->u.buf_init.pitch, sizeof(user_job_done->u.buf_init.pitch));
	memcpy(buf->buf_init.psize, user_job_done->u.buf_init.psize, sizeof(user_job_done->u.buf_init.psize));

	// TODO: user-job dma-attach
}
...
static int __buf_init(struct vb2_buffer *buffer) {
	...
	err = vmalloc_dma_map_sg(self->dev, vaddr, plane_size, &buf->sgt, DMA_FROM_DEVICE);
	if(err) {
...
static void __io_done(unsigned long cb_hndl, int err) {
	...
	if(atomic_read(&buf->io_done_pending) == 0) {
		atomic_set(&buf->io_done_pending, 1);
...
static void __io_done_work(struct work_struct* ws) {
	...
	atomic_set(&buf->io_done_pending, 0);
}
